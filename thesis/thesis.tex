...
However, the completion candidates that are proposed are sorted alphabetically. We believe, this approach ignores the potential of suggesting more relevant completion results based on their occurence in code history, which is something that has been successfully adopted by other IDEs. \oz{Problem statement is not convincing (see Section~\ref{sec:Introduction-Problem})}

Therefore, in this work we study how code completion can be improved \oz{by sorting completion results} using statistical language models \oz{\st{for sorting completion results}}. In particular, for our experiment we chose the n-gram language model. With the help of the n-gram implementation, we implement a sorting strategy where the most relevant results \oz{This makes little sense: \st{(based on likelihood analysis of source code)}} are shown first. This allows us to propose more accurate suggestions \oz{\st{even without knowing the exact type of the token} But you use the information about the type of token (e.g. the list of candidate completions that are passed to sorter were already selected based on their types)}, and makes finding a desired suggestion much faster.

\oz{\st{In the end, w}W}e \oz{\st{also quantitatively}} evaluate the various sorting strategies \oz{\st{implemented}} to be able to gauge performance accuracy for each one. \oz{I'm not sure I understand this sentence} 
\end{abstract}
