...
\noindent\enquote{Please do not be cynical. I hate cynicism. For the record, it's my least favorite quality. It's doesn't lead anywhere. No one in life gets exactly what they thought they were going to get. But if you work really hard and you're kind, amazing things will happen.}\bigbreak
...
The current implementation of code completion in the Pharo IDE is based on abstract syntax tree (AST) information of source code. This allows us to know the semantic role and, where possible, even the type of tokens. Hence, at every point in the completion, contextually relevant results are suggested.
...
Therefore, in this work we study how code completion can be improved by sorting completion results using statistical language models. In particular, for our experiment we chose the n-gram language model. With the help of the n-gram implementation, we implement a sorting strategy where the most relevant results are shown first. This allows us to propose more accurate suggestions even when we don't know the exact type of the token, and makes finding a desired suggestion much faster.

Finally, we evaluate the various sorting strategies to measure how effective they are in practice (i.e. we want to have relevance in addition to accuracy and speed).
\end{abstract}
