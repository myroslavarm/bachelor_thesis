...
The current implementation of code completion in the Pharo IDE is based on the abstract syntax tree (AST) of source code which allows us to know the semantic role and the kind of tokens (e.g. class name, method name, literal, etc.) and suggest contextually relevant completions. However, the current implementation has no effective way to sort the completion candidates, which means that sometimes the user must scroll through a long list of proposed completions to find the one that they need.

In this work, I study how code completion can be improved by sorting proposed completions in such a way that most frequently used tokens appear first. To that end, I implement a sorting plugin based on the n-gram language model.

I use the quantitative and qualitative evaluation techniques to compare the performance of the unigram,  bigram, and alphabetic sorter. As a result of the evaluation, the unigram sorter had the best performance.

The plugin with the n-gram based sorting strategies is open source and is available at \url{https://github.com/myroslavarm/CompletionSorting}.
\end{abstract}
