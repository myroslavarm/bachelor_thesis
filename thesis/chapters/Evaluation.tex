...
The following results in the Table~\ref{table:qual8} (below) are quite straightforward: all the sorters seem to perform well, although since one is less likely to want to call \textit{super} directly inside a block, the alphabetic one seems to be most relevant (purely coincidentally, as \textit{p} follows \textit{m}).
\begin{table}[H]
    \centering
    \begin{tabular}{llr}
    \hline
